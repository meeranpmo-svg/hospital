...
import { ShoppingCart, Building2, Plus, Send, CheckCircle2, XCircle, Trash2, FileText, Eye } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl bg-gradient-to-r from-indigo-700 to-blue-700 text-white p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-white/90 text-sm">Vendors · Purchase Orders · Approval workflow</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Stat label="PO Value (SAR)"    value={state.purchaseOrders.reduce((s, p) => s + p.total, 0).toLocaleString()} color="emerald" />
      </div>
...
function PurchaseOrders({ state, refresh }) {
  ...
  const markReceived = (id) => {
    ...
    // Add a GRN
    s.grns.push({
      id: uid('grn'), grnNo: `GRN-2026-${String(s.grns.length + 1).padStart(4,'0')}`,
        ...
        <table className="w-full">
          <thead>
            <tr className="text-xs text-slate-500 border-b border-slate-200">
              <th className="ltr:text-left rtl:text-right py-2 px-3">PO No.</th>
              <th className="ltr:text-left rtl:text-right py-2 px-3">Date</th>
              <th className="ltr:text-left rtl:text-right py-2 px-3">Vendor</th>
              <th className="ltr:text-left rtl:text-right py-2 px-3">Total (incl VAT)</th>
              <th className="ltr:text-left rtl:text-right py-2 px-3">Approval</th>
            ...
            {state.purchaseOrders.slice().reverse().map(po => {
              ...
              return (
                <tr key={po.id} className="border-b border-slate-100 hover:bg-slate-50">
                  <td className="py-2 px-3 font-mono text-sm">{po.poNo}</td>
                  <td className="py-2 px-3 text-sm">{po.date}</td>
                  <td className="py-2 px-3 font-medium">{v?.name}</td>
                  <td className="py-2 px-3 font-semibold">{(po.total + po.vat).toLocaleString()} SAR</td>
                  <td className="py-2 px-3"><span className={`badge ${apClass}`}>{po.approval}</span></td>
                  <td className="py-2 px-3"><span className={`badge ${stClass}`}>{po.status}</span></td>
          ...
          <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-3">
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div><div className="text-slate-500 text-xs">Approval</div><div className="font-medium capitalize">{view.approval}</div></div>
                <div><div className="text-slate-500 text-xs">Status</div><div className="font-medium capitalize">{view.status}</div></div>
              </div>
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-xs text-slate-500 border-b">
                    <th className="ltr:text-left rtl:text-right py-2">Item</th>
                    <th className="ltr:text-left rtl:text-right py-2 w-16">Qty</th>
                    <th className="ltr:text-right rtl:text-left py-2 w-24">Unit</th>
                    <th className="ltr:text-right rtl:text-left py-2 w-24">Total</th>
                  </tr>
                </thead>
                <tbody>
                  {view.items.map((it, i) => (
                    <tr key={i} className="border-b border-slate-100">
                      ...
                      <td className="py-2 ltr:text-right rtl:text-left">{it.unitPrice}</td>
                      <td className="py-2 ltr:text-right rtl:text-left font-medium">{(it.qty * it.unitPrice).toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              <div className="ltr:text-right rtl:text-left text-sm space-y-0.5 pt-2 border-t">
                <div>Subtotal: <b>{view.total.toLocaleString()} SAR</b></div>
                <div>VAT (15%): <b>{view.vat.toLocaleString()} SAR</b></div>
                <div className="text-lg">Total: <b>{(view.total + view.vat).toLocaleString()} SAR</b></div>
              </div>
      ...
      <table className="w-full">
        <thead>
          <tr className="text-xs text-slate-500 border-b border-slate-200">
            ...
            <th className="ltr:text-left rtl:text-right py-2 px-3">VAT No.</th>
            <th className="ltr:text-left rtl:text-right py-2 px-3">Terms</th>
        ...
        <tbody>
          {state.vendors.map(v => (
            <tr key={v.id} className="border-b border-slate-100 hover:bg-slate-50">
              <td className="py-2 px-3"><div className="font-medium">{v.name}</div><div className="text-xs text-slate-500" dir="rtl">{v.nameAr}</div></td>
              ...
              <td className="py-2 px-3 font-mono text-xs">{v.vatNo}</td>
              <td className="py-2 px-3 text-sm">{v.paymentTerms}</td>
              <td className="py-2 px-3 ltr:text-right rtl:text-left font-semibold text-rose-700">{v.balance > 0 ? `${v.balance.toLocaleString()} SAR` : '—'}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function NewPO({ state, refresh, setTab }) {
  ...
  const submit = () => {
    ...
    s.purchaseOrders.push({
      id: uid('po'),
      poNo: `PO-2026-${String(s.purchaseOrders.length + 1).padStart(4,'0')}`,
      vendorId, date: new Date().toISOString().slice(0,10),
      status: 'pending', approval: subtotal > 50000 ? 'pending' : 'approved',
      total: subtotal, vat,
      items: items.filter(x => x.name).map(x => ({ name: x.name, qty: Number(x.qty), unitPrice: Number(x.unitPrice) })),
      requestedBy: 'u12', approvedBy: subtotal > 50000 ? null : 'u1', notes,
    });
    saveState(s);
    setTab('po'); refresh();
  };
  ...
  return (
    <div className="card">
      ...
      <div className="space-y-2">
        {items.map((it, i) => (
          <div key={i} className="grid grid-cols-12 gap-2">
            ...
            <input type="number" className="input col-span-2" placeholder="Unit price" value={it.unitPrice} onChange={e => setItem(i,'unitPrice',e.target.value)} />
      ...
      <div className="mt-4 p-4 bg-slate-50 rounded-lg ltr:text-right rtl:text-left">
        <div>Subtotal: <b>{subtotal.toLocaleString()} SAR</b></div>
        <div>VAT (15%): <b>{vat.toLocaleString()} SAR</b></div>
        <div className="text-lg mt-1">Total: <b>{total.toLocaleString()} SAR</b></div>
        {subtotal > 50000 && <div className="text-xs text-amber-700 mt-1">⚠️ Over 50,000 SAR — requires admin approval</div>}
      </div>
